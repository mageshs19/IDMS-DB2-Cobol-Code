...
class SqlGenerator:
    def __init__(
        ...
        self.rows_by_record = self._group_rows_by_record(rows)
    ...
    def select_by_key(self, record_name: str) -> list[str]:
        ...
        table = self._best_table_for_rows(rows)

        if not table:
            return self._todo(f"Missing DB2 table mapping for {record_name}")

        columns = self._mapped_columns(rows=rows, table_name=table)

        if not columns:
            return self._todo(f"Missing DB2 column mapping for {record_name}")

        hosts = self._host_variables(rows=rows, table_name=table)

        if not hosts:
            return self._todo(f"Missing host variable mapping for {record_name}")
        ...
        return [
            "           EXEC SQL",
            "                SELECT",
            *self._comma_lines(columns, "                    "),
            "                INTO",
            *self._comma_lines(hosts, "                    "),
            f"                FROM {table}",
            "                WHERE",
            *self._and_lines(
                self._where_conditions(rows=key_rows, table_name=table),
                "                    ",
            ),
            "           END-EXEC.",
        ]
    ...
    def insert(self, record_name: str) -> list[str]:
        ...
        if not table or not columns or not hosts:
            return self._todo(f"Missing INSERT mapping for {record_name}")
    ...
    def update(self, record_name: str) -> list[str]:
        rows = self._record_rows(record_name)
        table = self._best_table_for_rows(rows)

        if not table:
            return self._todo(f"Missing UPDATE mapping for {record_name}")
        ...
        for row in rows:
            ...
            host = self._host_for_row(row=row, table_name=table)
        ...
        if not set_lines:
            set_lines.append("/* TODO: Add update columns */")
    ...
    def delete(self, record_name: str) -> list[str]:
        ...
        if not table:
            return self._todo(f"Missing DELETE mapping for {record_name}")
    ...
    def cursor_name(self, set_name: str) -> str:
        normalized = NameNormalizer.normalize(set_name)

        if not normalized:
            return "C-IDMS-SET"

        return "C-" + NameNormalizer.to_cobol(normalized)

    def open_paragraph_name(self, set_name: str) -> str:
        return "OPEN-" + self.cursor_name(set_name)

    def fetch_paragraph_name(self, set_name: str) -> str:
        return "FETCH-" + self.cursor_name(set_name)

    def close_paragraph_name(self, set_name: str) -> str:
        return "CLOSE-" + self.cursor_name(set_name)

    def open_cursor(self, set_name: str) -> list[str]:
        return [f"           PERFORM {self.open_paragraph_name(set_name)}"]

    def fetch_cursor(self, record_name: str, set_name: str) -> list[str]:
        return [f"           PERFORM {self.fetch_paragraph_name(set_name)}"]

    def close_cursor(self, set_name: str) -> list[str]:
        return [f"           PERFORM {self.close_paragraph_name(set_name)}"]

    def has_cursor_relationship_condition(self, record_name: str, set_name: str) -> bool:
        return bool(
            self.cursor_where_conditions(record_name=record_name, set_name=set_name)
        )

    def cursor_declare(self, record_name: str, set_name: str) -> list[str]:
        rows = self._record_rows(record_name)
        table = self._best_table_for_rows(rows)
        columns = self._mapped_columns(rows=rows, table_name=table)
    ...
    def db2_infrastructure_block(
        ...
        for set_name, record_name in used_cursor_records.items():
            lines.extend(
                self.cursor_declare(record_name=record_name, set_name=set_name)
            )
            lines.append("")
    ...
    def cursor_paragraph_block(
        ...
        for set_name, record_name in used_cursor_records.items():
            cursor_name = self.cursor_name(set_name)
            open_paragraph = self.open_paragraph_name(set_name)
            fetch_paragraph = self.fetch_paragraph_name(set_name)
            close_paragraph = self.close_paragraph_name(set_name)
    ...
    def cursor_where_conditions(self, record_name: str, set_name: str) -> list[str]:
        child_rows = self._record_rows(record_name)
        child_table = self._best_table_for_rows(child_rows)
        ...
        for row in relationship_rows:
            child_column = NameNormalizer.normalize(row.new_db2_field_name)

            if not child_column:
                continue

            parent_host = self._parent_host_for_relation_row(row)
            ...
            if not parent_host:
                ...
                parent_host = ":" + NameNormalizer.to_cobol(parent_reference)
    ...
    def cursor_order_by_columns(
        ...
        for row in self._key_rows(child_rows):
            column = NameNormalizer.normalize(row.new_db2_field_name)
    ...
    def _relationship_rows_for_cursor(
        ...
        normalized_set = NameNormalizer.normalize(set_name)
        normalized_record = NameNormalizer.normalize(record_name)
        normalized_record_no_suffix = NameNormalizer.remove_record_suffix(normalized_record)
        normalized_child_table = NameNormalizer.normalize(child_table)
        ...
        for row in self.rows:
            ...
            row_record_no_suffix = NameNormalizer.remove_record_suffix(row_record)
            row_table = NameNormalizer.normalize(row.new_db2_record)
    ...
    def _parent_host_for_relation_row(self, row: SheetMappingRow) -> str:
        parent_table = NameNormalizer.normalize(row.cross_application_db2_table)
        parent_column = NameNormalizer.normalize(row.cross_application_db2_field_name)

        if parent_table and parent_column:
            host = self.dclgen_host_lookup.get((parent_table, parent_column))
        ...
        if reference_field:
            mapped_parent = self._find_mapping_by_source_field(reference_field)

            if mapped_parent is not None:
                mapped_table = NameNormalizer.normalize(mapped_parent.new_db2_record)
                mapped_column = NameNormalizer.normalize(mapped_parent.new_db2_field_name)

                host = self.dclgen_host_lookup.get((mapped_table, mapped_column))
        ...
        fallback_column = NameNormalizer.normalize(row.new_db2_field_name)
        host = self.dclgen_host_lookup.get(("", fallback_column))
    ...
    def _find_mapping_by_source_field(
        ...
        normalized_source = NameNormalizer.normalize(source_field)

        for row in self.rows:
            row_source = self._source_field_from_cobol_zone(row.cobol_zone)

            if NameNormalizer.normalize(row_source) == normalized_source:
                if row.new_db2_field_name:
                    return row

        return None

    def _source_field_from_cobol_zone(self, value: str) -> str:
        ...
        if parts and parts[0].isdigit():
            if len(parts) > 1:
                return NameNormalizer.to_cobol(parts[1])
    ...
    def _group_rows_by_record(
        ...
        for row in rows:
            ...
            if no_suffix and no_suffix != record:
                grouped[no_suffix].append(row)

        return grouped

    def _record_rows(self, record_name: str) -> list[SheetMappingRow]:
        ...
        return self.rows_by_record.get(no_suffix, [])
    ...
    def _build_dclgen_host_lookup(
        ...
        for column in columns:
            table = NameNormalizer.normalize(column.table_name)
            db2_column = NameNormalizer.normalize(column.column_name)
            host = NameNormalizer.to_cobol(column.cobol_host_name or column.column_name)
            ...
            lookup[("", db2_column)] = host if not table else f"DCL{table}.{host}"

        return lookup

    def _dclgen_include_names(self) -> list[str]:
        names: list[str] = []
        seen: set[str] = set()
    ...
    def _best_table_for_rows(self, rows: list[SheetMappingRow]) -> str:
        ...
        for row in rows:
            ...
            if table and column:
                explicit_table_scores[table] = explicit_table_scores.get(table, 0) + 1

        if explicit_table_scores:
            return max(explicit_table_scores.items(), key=lambda item: item[1])[0]
    ...
    def _mapped_columns(
        ...
        normalized_table = NameNormalizer.normalize(table_name)

        dclgen_columns = {
            NameNormalizer.normalize(column.column_name)
            for column in self.dclgen_by_table.get(normalized_table, [])
        }
    ...
    def _host_variables(
        ...
        for row in rows:
            ...
            host = self._host_for_row(row=row, table_name=normalized_table)
    ...
    def _host_for_row(
        ...
        normalized_table = NameNormalizer.normalize(table_name)
        host = self.dclgen_host_lookup.get((normalized_table, column))
        ...
        host = self.dclgen_host_lookup.get(("", column))
    ...
    def _key_rows(self, rows: list[SheetMappingRow]) -> list[SheetMappingRow]:
        output: list[SheetMappingRow] = []

        for row in rows:
            if not NameNormalizer.normalize(row.new_db2_field_name):
                continue

            if NameNormalizer.normalize(row.idms_key) or NameNormalizer.normalize(row.db2_key):
                output.append(row)
    ...
    def _where_conditions(
        ...
        for row in rows:
            ...
            output.append(
                f"{column} = {self._host_for_row(row=row, table_name=table_name)}"
            )

        if not output:
            output.append("/* TODO: Add key condition */ 1 = 1")

        return output

    def _comma_lines(self, items: list[str], indent: str) -> list[str]:
        output: list[str] = []
    ...
    def _and_lines(self, items: list[str], indent: str) -> list[str]:
        output: list[str] = []
    ...
    def _todo(self, message: str) -> list[str]:
        return [
            f"           * TODO DB2: {message}.",
            "           CONTINUE.",
        ]