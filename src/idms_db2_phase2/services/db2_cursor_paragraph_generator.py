...
class Db2CursorParagraphGenerator:
    """
    Generates generic DB2 OPEN/FETCH/CLOSE paragraphs for converted cursor usage.

    This generator preserves existing comments and business paragraph names.
    It appends generated DB2 cursor paragraphs near the end of the program.

    It also replaces inline generated cursor SQL blocks with PERFORM calls where possible.

    Generated paragraph names:
        OPEN-C-SET-NAME
        FETCH-C-SET-NAME
        CLOSE-C-SET-NAME
    ...
    The generated FETCH paragraph uses DCLGEN host variables when DCLGEN columns
    are available. Otherwise it falls back to COBOL names derived from DB2 columns.
    """
    ...
    def apply(
        ...
        for spec in cursor_specs:
            updated_text = self._replace_inline_open_fetch_close(
                ...
                open_paragraph=spec["open_paragraph"],
                fetch_paragraph=spec["fetch_paragraph"],
                close_paragraph=spec["close_paragraph"],
            )

        paragraph_block = self._build_cursor_paragraph_block(
    ...
    def _build_cursor_paragraph_block(
        self,
        ...
        for spec in cursor_specs:
            lines.extend(
                self._open_paragraph(
                    ...
                    paragraph_name=str(spec["open_paragraph"]),
                )
            )
            ...
            lines.extend(
                self._fetch_paragraph(
                    cursor_name=str(spec["cursor_name"]),
                    paragraph_name=str(spec["fetch_paragraph"]),
            ...
            lines.extend(
                self._close_paragraph(
                    cursor_name=str(spec["cursor_name"]),
                    paragraph_name=str(spec["close_paragraph"]),
                )
            )
    ...
    def _fetch_host_variables(
        ...
        for row in rows:
            ...
            host_variable = self._host_variable_for_column(
                db2_table=db2_table,
                db2_column=db2_column,
            )

            if not host_variable:
                continue

            if host_variable in seen:
                continue
    ...
    def _host_variable_for_column(
        ...
        dclgen_host = self.dclgen_lookup.get(
            (
                db2_table,
                db2_column,
            )
        ...
        if dclgen_host:
            return f":{dclgen_host}"

        if db2_table:
            return f":DCL{db2_table}.{NameNormalizer.to_cobol(db2_column)}"

        return f":{NameNormalizer.to_cobol(db2_column)}"
    ...
    def _build_dclgen_lookup(
        ...
        for column in self.dclgen_columns:
            ...
            group_name = f"DCL{table_name}" if table_name else ""

            if group_name:
                host_reference = f"{group_name}.{host_name}"
            else:
                host_reference = host_name
    ...
    def _rows_for_record(
        ...
        normalized_no_suffix = NameNormalizer.remove_record_suffix(
            normalized_record,
        )
