...
# C:\VSCode\IDMS-DB2-Code-Conversion\src\idms_db2_phase2\testing\run_update.py
#
...
# Main update program source.
# Change this file name if your local update source has another name.
IDMS_COBOL_SOURCE_PATH = INPUT_DIR / "Update.txt"

# Update program target.
TARGET_PROGRAM_ID = "VMDZ1567"

# Keep False unless you explicitly want output PIC lengths auto-expanded.
...
# DCLGEN files
# ---------------------------------------------------------------------
# For the VMDZ1567 update sample, BFAR is required.
#
# BEFF/EVEF are included as optional candidates because your input folder
# may contain common DCLGEN sets. The runner loads only files that exist.
#
# If you have a restart/control DCLGEN for FFRECAB / DZ01RSTV, add it here,
# for example:
#     INPUT_DIR / "DCLGENs_01RS.txt",
#     INPUT_DIR / "DCLGENs_RSTV.txt",
# ---------------------------------------------------------------------

DCLGEN_CANDIDATE_PATHS = [
...
DCLGEN_PATHS = [
    path
    for path in DCLGEN_CANDIDATE_PATHS
...
# ---------------------------------------------------------------------
# Add copybooks here if available.
#
# Examples:
# COPYBOOK_PATHS = [
#     INPUT_DIR / "VMBD205I.cpy",
# ]
# ---------------------------------------------------------------------

COPYBOOK_CANDIDATE_PATHS = [
    INPUT_DIR / "Copybook.txt",
]

COPYBOOK_PATHS = [
    path
    for path in COPYBOOK_CANDIDATE_PATHS
    if path.exists() and path.is_file()
]
...
# Example: Update_12-08-2026_153045.cbl
# ---------------------------------------------------------------------
...
def validate_inputs() -> None:
    ...
    if not DCLGEN_PATHS:
        searched = "\n".join(str(path) for path in DCLGEN_CANDIDATE_PATHS)
        raise ValueError(
